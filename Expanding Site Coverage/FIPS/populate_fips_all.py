#!/usr/bin/env python3
"""
Populate gov_level, state_fips, and county_fips for ALL US rows (not just active ones)
in Copy of AW_civic_scraper_sites.xlsx and Copy_of_Legistrar_sites.xlsx.

This is a variant of populate_fips.py with the active-row filters removed.
...
AW_OUT    = f"{BASE}/FIPSupdated_AW_civic_scraper_sites.xlsx"
LEG_OUT   = f"{BASE}/FIPSupdated_Legistrar_sites.xlsx"
AW_NOTES  = f"{BASE}/FIPSupdated_AW_civic_scraper_sites_FIPS_notes.txt"
LEG_NOTES = f"{BASE}/FIPSupdated_Legistrar_sites_FIPS_notes.txt"
...
def process_aw(county_lkp, place_lkp):
    ...
    for idx, row in df.iterrows():
        # NOTE: active-only filter removed — process ALL rows
        ...
        # For rows without gov_level, we still attempt county_fips resolution
        # via the place lookup fallback inside resolve_county_fips. The function
        # will skip the county-name-specific branch (which requires gov_level='county')
        # and fall through to the place->county crosswalk, which handles
        # municipalities and most other entity types.
...
AW_WORKFLOW = """\
WORKFLOW — AW_civic_scraper_sites (ALL rows, not just active)
--------------------------------------------------------------
1. Process ALL rows where state is a US state abbreviation (aw_active filter removed).
...
3. county_fips (stored as integer representing the full 5-digit FIPS, leading
   ...
   c. For gov_level = 'municipality', 'other', or empty: look up place name
      against the Census 2020 Place-County relationship file
      (national_place_by_county2020.txt). When a place spans multiple
...
LEG_WORKFLOW = """\
WORKFLOW — Legistrar_sites (ALL rows, not just active)
-------------------------------------------------------
1. Process ALL rows where state is a US state abbreviation (aw_active filter removed).
...
if __name__ == '__main__':
    ...
    write_notes(AW_NOTES, "AW_civic_scraper_sites — FIPS Population Notes (ALL rows)",
    ...
    write_notes(LEG_NOTES, "Legistrar_sites — FIPS Population Notes (ALL rows)",
